...
def check_logs(log_dir):
    ...
    debate_files = [f for f in files if os.path.basename(f).startswith("debate")]
    ...
    no_consensus = find_no_consensus(debate_files)

    results["no_initial_consensus"] = len(no_consensus)

    return results, misclassified, no_consensus
...
def print_no_consensus(no_consensus):

    if len(no_consensus) == 0:
        print("\nAll debates had initial consensus.")
        return

    print("\nDebates WITHOUT Initial Consensus")
    print("---------------------------------\n")

    for item in no_consensus[:20]:
        print("File:", item["file"])
        print("Question:", item["question"])
...
# --------------------------------------------------
# Find debates without initial consensus
# --------------------------------------------------

def find_no_consensus(debate_files):
    ...
    for file in debate_files:
        ...
        if data.get("consensus") is False:
            no_consensus.append({
...
def main():
    ...
    results, misclassified, no_consensus = check_logs(log_dir)
    ...
    print_no_consensus(no_consensus)
